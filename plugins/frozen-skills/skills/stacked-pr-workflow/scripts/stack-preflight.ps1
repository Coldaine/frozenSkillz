...
$trunk = $null
$remoteHead = (Invoke-Git -GitArgs @('ls-remote','--symref','origin','HEAD') | Select-Object -First 1)
if ($remoteHead -and $remoteHead -match '^ref:\s+refs/heads/(?<branch>\S+)\s+HEAD$') {
  $trunk = $Matches.branch
}
if (-not $trunk) {
