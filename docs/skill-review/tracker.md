...
| 1. luxcd/agent-skills → **gitops-repo-audit only** (200 stars) | adopt · adapt | _(not snapshotted)_ | Static repo audit / validate / report. Skip gitops-cluster-debug and the rest of the Flux package. Wire Coldaine P0 / intentional apply afterward. |
...
| Parked / do not adopt as skills | discarded · noted 2026-07-21 | — | Whole [Aidas-dev/k8s-agent-skills](https://github.com/Aidas-dev/k8s-agent-skills) dump (1 star) — mine CNPG CRD table later if needed; Talos skill is apply-typist. lux router + gitops-cluster-debug. 
