...
| **Adopt shortlist (2026-07-21)** — premise corrected 2026-07-23: reconciler is Flux, not Helmfile | decision · pending confirm | — | **Correction:** `coldaine-k8cluster` is retired; its declared active successor `coldaine-homelab` runs Flux exclusively (`Git → Flux → CAPI → CAPMOX → Proxmox VMs`, decision `gitops-flux-capi`, with an explicit rejection of a second Argo/Helmfile reconciliation plane). The 2026-07-21 premise ("apply path is helmfile apply") never checked the successor repo, which had already been created six days earlier. Do **not** intake whole skill repos, regardless of reconciler. The live re-evaluation of the Flux-based stack (`fluxcd/agent-skills`, `kubetail-org/kstack`, `containers/kubernetes-mcp-server`, a CAPMOX-specific glue skill, HolmesGPT) is tracked in [coldaine-homelab#92](https://github.com/Coldaine/coldaine-homelab/issues/92), not here — that issue supersedes rows below on the reconciler question. |
| 1. ~~helmfile/helmfile → skills/helmfile scoped (5162 stars)~~ | **stale — wrong reconciler** | _(not snapshotted)_ | Targeted Helmfile diff/doctor/intentional-apply language, but coldaine-homelab (the active successor) reconciles via Flux, not Helmfile. Superseded by the Flux-stack evaluation in coldaine-homelab#92; do not adapt this source for the current architecture. |
| 2. LukasNiessen/kubernetes-skill → **core workflow + failure-mode refs** (324 stars) | adopt · adapt | _(not snapshotted)_ | Manifest a prove-before-mutate layer ahead of whichever apply path is actually live (Flux reconcile today, not Helmfile commit/apply — see correction above). Drop cloud CRR dumps (EKS/GKE/AKS) unless a real intent appears. |
| 3. Coldaine **k8s-platform-operator** (author) + CAPMOX seed | author · design | _(not authored)_ | Thin, reconciler-agnostic glue skill: prove → diff (Flux/Kustomize today; treat the reconciler as a pluggable detail, not a hard dependency) → P0 → intentional apply → live verify. Seed CAPMOX from [ionos-cloud/cluster-api-provider-proxmox AGENTS.md](https://github.com/ionos-cloud/cluster-api-provider-proxmox/blob/main/AGENTS.md) (463 stars), adapted for behavior shape only (declare the complete target at once, no human-staged scale gates, never synthesize ownership over pre-existing resources just because a provider ID matches, record known provider defects explicitly). Not an external install; do not import homelab-specific facts already covered by `infra/k8s/capmox/SKILL.md` in coldaine-homelab. |
...
| Reopened for scoring (2026-07-23) | was: discarded · noted 2026-07-21 | — | **Flux skills** (`fluxcd/agent-skills`, Aidas Flux router, `gitops-cluster-debug`) were parked as "wrong reconciler for coldaine-k8cluster" — that premise was false for the active successor. Re-scored in [coldaine-homelab#92](https://github.com/Coldaine/coldaine-homelab/issues/92) (closed 2026-07-25): `fluxcd/agent-skills` adopt-pinned; `gitops-cluster-debug` fork-not-adopt-raw (hard-requires `flux-operator-mcp`/`FluxInstance`, and coldaine-homelab runs a plain `flux bootstrap` install with no `FluxInstance` CRD — a confirmed defect for this repo); `kstack` vendor-selectively (its ask-before-every-mutation default and 15-minute cache don't fit a convergence loop). Still parked regardless of reconciler: whole Aidas dump; kubectl-MCP packs; clouddrove/Jeffallan/sickn33/wshobson mutate cookbooks; Omni-as-CAPMOX; kagent apply-after-generate (kagent: revisit only after CAPI/CAPMOX/Flux is stable). |

Scout note (corrected 2026-07-23): the earlier "Flux gitops-repo-audit ranking was a stack mismatch" call was itself the mismatch — coldaine-homelab reconciles via Flux. Authored-vs-applied still matters; implement it against Flux Kustomizations/HelmReleases, not Helmfile.
