...
| `agent-atlas` (renamed from `agent-tool-config-router` 2026-07-07) | authored 2026-07-07 | 🛑 gated · evaluate | Verify against `D:\_projects\coldaine-configurations\configurations\2026-07-07-agent-tool-configuration-policy.md`; decide whether it stays local-only or can be de-personalized for active promotion. Per-tool references added 2026-07-07 (codex, cursor, gemini/antigravity, opencode, kilo, copilot-cli, goose, qwen, openrouter), live-verified drafts; `references/research-todo.md` is the fill-in contract for a future research agent. Open blocker: canonical Kilo root (`.kilo` vs `.kilocode`). |
