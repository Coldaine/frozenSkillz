...
| `fluxcd/agent-skills` → `gitops-repo-audit` (+ knowledge router) (`200` stars, 2026-07-21) | scout · evaluation pending | _(not snapshotted)_ | Open `_incubator/scout/` via `external-skill-intake`: authored-vs-applied separation is the best external anti-typist match. Scope to repo audit / validate / report; do **not** promote `gitops-cluster-debug` MCP apply paths without break-glass. Pair with Coldaine P0 gates. |
| `LukasNiessen/kubernetes-skill` (`324` stars, 2026-07-21) | scout · evaluation pending | _(not snapshotted)_ | Snapshot failure-mode / anti-hallucination manifest workflow; adapt progressive `references/`; subordinate to GitOps apply path (“status means reconciled, not generated”). |
| `Aidas-dev/k8s-agent-skills` → `talos` + `cnpg` + `flux` router only (`1` star, 2026-07-21) | scout · evaluation pending | _(not snapshotted)_ | Closest public Talos/CNPG/Flux homelab skill set. Scope narrowly — not the full ~50-skill dump. Strip imperative apply/upgrade defaults; wire to reconciled-not-pushed gates. |
| `siderolabs/docs` `public/skill.md` (`11` stars, 2026-07-21) | scout · evaluation pending | _(not snapshotted)_ | Canonical Talos agent contract; concept-borrow wcygan read-only talosctl guardrails (`wcygan/dotfiles`, `192` stars). Prefer over Aidas Talos for authority; use Aidas for stack adjacency. |
| Coldaine `k8s-platform-operator` (author) + CAPMOX seed | gated · design pending | _(not authored)_ | Home-grown thin skill: prove → diff → P0 → intentional apply → live verify. Seed CAPMOX from [ionos-cloud/cluster-api-provider-proxmox `AGENTS.md`](https://github.com/ionos-cloud/cluster-api-provider-proxmox/blob/main/AGENTS.md) (`463` stars) — no public CAPMOX `SKILL.md` exists. Concept-mine faberlens SAFETY (`24` stars), helmfile official skill diff/doctor only (`5161` stars), and ConfigHub ApplyGates language (`confighub/confighub-skills`, `2` stars) without adopting ConfigHub. |
| Reject queue (do not scout as default mutate skills) | discarded · noted 2026-07-21 | — | `rohitg00/kubectl-mcp-server` kubernetes-skills + live kubectl MCP toolsets; clouddrove/Jeffallan/sickn33 generic k8s deploy/architect dumps; wshobson `gitops-workflow` (`kubectl apply` / force sync); flux `gitops-cluster-debug` as default mutate; Omni+Proxmox as CAPMOX substitute; kagent apply-after-generate patterns; AWS EKS create/deploy dumps. |

Scout ranking distilled 2026-07-21 from AgentsView k8s-typist lessons plus intake passes (operator skills + GitOps safety + Talos/CAPMOX). No external skill fully matches Helmfile + P0 + propose-only `cluster-status`; adapt, don’t promote wholesale.
