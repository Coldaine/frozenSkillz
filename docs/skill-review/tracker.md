...
| `obra/superpowers` `v6.1.1` (`d884ae04edebef577e82ff7c4e143debd0bbec99`) | scout · forensic review in progress | `_incubator/scout/2026-07-23-obra-superpowers/` | Pinned 172-file source and 14-skill doctree. `brainstorming` reviewed B- with moderate confidence. `dispatching-parallel-agents` is revised to B- after an AgentsView audit found direct successful and failed Codex uses; confidence is strong for observed Codex behavior and moderate cross-harness. Remaining 12 skills are intentionally ungraded pending one-at-a-time review. |
...
`_incubator/scout/`, requires inventory, artifact rubrics, live or forensic evaluation evidence, and a packaging
decision before any candidate idea moves into active marketplace content.
