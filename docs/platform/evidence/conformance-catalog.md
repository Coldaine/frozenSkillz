...
| `AC-RUN-004` | Concurrent-client tests identify process/container reuse, independent authorization transactions, credential isolation, memory use, log attribution, and orphan cleanup. | `pasted-text.txt:L758-771`; supplement multiplexer proposal at `supplement:L17` | `REQ-012`, `REQ-029` | `source-explicit`, `decision-needed`; pass thresholds and sharing policy missing |
...
| `AC-RUN-007` | Under an approved hard-authorization policy, a direct disallowed `tools/call` receives the approved protocol-valid denial and backend evidence proves the call was never dispatched. A dispatch-policy load/parse failure follows the approved fail-closed behavior. | Supplement at `supplement:L17` | `REQ-028` | `deferred post-v1`; `decision-needed`; requires denial/audit contract |
| `AC-RUN-008` | Under approved discovery filtering, every page of `tools/list` and every refresh after a tool-list-change notification contains all and only allowed tools, without omissions, duplicates, or policy bypass. | Supplement at `supplement:L17`; current MCP tools verification | `REQ-028` | `deferred post-v1`; `decision-needed`; pagination/change fixtures needed |
| `AC-RUN-009` | Two simultaneous client connections remain isolated when a process/container is reused: identical request IDs, different capabilities/roots/auth contexts, cancellation, callbacks, notifications, logging changes, disconnects, and shutdown cannot cross sessions. | Supplement multiplexer proposal at `supplement:L17`; current MCP lifecycle/architecture verification | `REQ-012`, `REQ-029` | `deferred post-v1`; `decision-needed`; backend-sharing eligibility open |
| `AC-RUN-010` | Under the approved containment policy, the selected runtime root is canonicalized across platform links/aliases and backend access is refused when it lies outside the explicitly authorized set. Client roots do not prove OS sandboxing. | Supplement root proposal at `supplement:L17`, corrected by security review | `REQ-030` | `deferred post-v1`; `decision-needed`; authorized-root interface needed |
| `AC-RUN-011` | Concurrent HTTP authorization flows use unique single-use state and per-flow PKCE, bind tokens to the intended resource/audience/identity/scope/consent, store them under the approved secret policy, and never pass the client-presented token to a downstream resource; the proxy obtains a separate downstream token. Stdio credential handling remains a distinct machine contract. | Derived from the supplement's OAuth reuse claim at `supplement:L17` and current MCP authorization guidance | `REQ-029` | `deferred post-v1`; `decision-needed`; auth model and reusable-backend policy open |
| `AC-RUN-012` | Roots are requested only after a client advertised support during initialization. A later root-list change that removes or narrows an active root follows the approved constrain/restart/terminate policy and never leaves stale broader access. | Current MCP lifecycle/roots verification | `REQ-011`, `REQ-030` | `deferred post-v1`; `decision-needed`; root lifecycle open |
