...
| IF-12 | CLI contract (deferred post-v1) | User/CI boundary for lock, materialization, validation, MCP execution, and scanning | Syntax, stdin/stdout/stderr, JSON modes, exit codes, dry-run, conflict-resolution flow, transactional guarantees | `frozen` versus `frozenctl`; public verbs; whether `render` is internal to `sync`; compatibility aliases |
