...
| `FI-REQ-003` | On Windows or Linux, remove the selected host launcher, omit its machine mapping, or map it to a missing executable. | Host MCP execution reports a stable unavailable classification, launches no guessed or fallback executable, and records configuration present / launcher unavailable / runtime untested. | `REQ-010`, `REQ-014`; `AC-PLAT-004` | Error and observation schemas (`GAP-011`) |
...
## Supplemental seven-case profile crosswalk

The second attachment names this as an exact minimum profile. The crosswalk preserves
its names without replacing the broader catalog or approving conditional outcomes.

| Supplemental case | Catalog case | Reconciliation |
|---|---|---|
| Mutated Vendor Test | `FI-REQ-001` | Validation must report drift; synchronization must enter the explicit non-destructive conflict path. |
| Collision Test | `FI-REQ-002` | Parameterize a project-versus-global collision and report the effective client-specific winner. |
| Missing Shim Test | `FI-REQ-003` | Covers both Windows and Linux, missing machine mapping, and mapping to a missing executable. |
| Kilo Merge Test | `FI-REQ-006` | Preserve unrelated semantics; byte/comment/order guarantees remain blocked by `GAP-007`. |
| Multiplexer Memory Test | `FI-GAP-014` | The process/container count assertion follows the approved backend-sharing policy; one backend is not assumed universally. |
| Tool Jailbreak Test | `FI-GAP-030` | Applies only if hard call-time authorization is approved; discovery hiding alone cannot pass it. |
| Path Traversal Test | `FI-GAP-032` | Uses canonicalized authorized-root containment rather than the supplement's fixed deny-list. |
...
| `FI-GAP-014` | Launch the same gateway from multiple supported clients concurrently, including an OAuth-backed downstream server. | Evidence records the declared process/container reuse behavior, independent authorization transactions, credential/session isolation, memory, latency, log attribution, and cleanup. Exactly one backend is required only if that backend is explicitly approved as shareable. | `REQ-012`, `REQ-029`; `AC-RUN-003`, `AC-RUN-004`, `AC-RUN-009` | `decision-needed`; sharing policy and budgets open |
...
| `FI-GAP-030` | Bypass filtered discovery and send a disallowed `tools/call` directly through the managed proxy. | Under an approved hard-authorization model, the proxy returns the stable protocol-valid denial before dispatch; backend evidence records zero receipt and no tool side effect. | `REQ-028`; `AC-RUN-007` | `decision-needed` |
| `FI-GAP-031` | Two clients sharing an eligible backend use the same request ID while negotiating different capabilities/roots; then one cancels and disconnects. | Each response, callback, notification, cancellation, log setting, and cleanup event remains bound to its originating session; the other client remains healthy. | `REQ-029`; `AC-RUN-009` | `decision-needed`; only runnable for an approved sharing model |
| `FI-GAP-032` | Supply a selected runtime root that escapes through `..`, a symlink, Windows junction/reparse point, UNC/device path, drive/case alias, encoded traversal, or mount boundary. | Under the approved containment policy, canonicalized containment fails before backend access; no deny-list match is required, no unsafe process starts, and diagnostics expose no sensitive filesystem inventory. | `REQ-030`; `AC-RUN-010` | `decision-needed`; platform fixtures needed |
| `FI-GAP-033` | Return a paginated tool list containing denied tools, then emit a tool-list-change notification that adds another denied tool. | Under approved discovery filtering, every client-visible page and refresh remains complete for allowed tools and contains no denied tool; a policy failure follows the approved fail-closed behavior rather than exposing the raw backend list. | `REQ-028`; `AC-RUN-008` | `decision-needed` |
| `FI-GAP-034` | Start concurrent HTTP authorization flows, attempt to reuse state or PKCE material, and attempt to pass the client-presented token to the downstream resource. | Every flow has unique single-use state and PKCE; the proxy never passes through the received token and instead obtains a separate resource/audience-bound downstream token under the approved identity/scope/consent policy; durable evidence contains no token or credential value. | `REQ-029`; `AC-RUN-011` | `decision-needed`; HTTP auth model open |
| `FI-GAP-035` | After initialization, mutate the client's root set to remove or narrow the active root, emit `notifications/roots/list_changed`, and return the narrowed set when the proxy issues a new `roots/list` request. | The approved constrain/restart/terminate behavior takes effect before further out-of-scope access; the session never continues silently under the stale broader root set. | `REQ-011`, `REQ-030`; `AC-RUN-012` | `decision-needed`; root lifecycle open |
