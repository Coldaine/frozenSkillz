...
| `REQ-011` | MCP project-root selection uses an explicit ordered contract rather than assuming cwd. Candidate inputs include managed invocation identity, post-initialization roots from a capable client, verified client-specific values, shim state, and constrained cwd inference; the contract defines multi-root selection, later root-list changes, and explicit failure when no intended root is unambiguous. | `pasted-text.txt:L544-563`; supplement root proposal at `supplement:L17`, reconciled with current MCP lifecycle | `candidate`, `open` | Runtime interface and per-client conformance. Select the order and lifecycle independently from the authorization/containment rule in `REQ-030`. |
...
| `REQ-026` | A fresh checkout should contain the committed native configuration needed for the declared project clients before running a local renderer, while machine-local trust, credentials, launchers, and health may still be pending. | Assistant-attributed prior-user requirement at `pasted-text.txt:L264-295`, especially `pasted-text.txt:L279-281`; repeated by the supplement at `supplement:L1` | `indirect-missing-context`, `confirmation-needed` | Confirm with user, then define fresh-clone acceptance separate from activation. The supplement's stronger "immediately functional" wording is not approved. |
...
| `REQ-028` | Managed server-tool discovery filtering and server-tool call-time authorization are separately selectable guarantees. If discovery filtering is approved, every paginated result and refresh is filtered under its fail-closed policy. If hard authorization is approved, disallowed calls are rejected before backend dispatch with protocol-valid redacted errors and proof of zero backend delivery. | First-source exposure policy at `pasted-text.txt:L773-793`; supplement discovery/call interception at `supplement:L17` | `open` | MCP policy/session interface, security architecture, conformance, and failure injection. Decide whether sampling-driven tool use, prompts, resources, and unmanaged/direct routes are outside v1 policy or separately mediated. |
| `REQ-029` | Every managed MCP client-to-proxy/backend relationship retains an independent stateful protocol session. A process or container may be reused only when the backend's multi-session behavior is declared and proven to isolate initialization, capabilities, IDs, callbacks, roots, transport-appropriate auth/consent, cancellation, notifications, logging, and shutdown. | Supplement daemon/multiplexer proposal at `supplement:L17`; current MCP lifecycle/architecture verification | `candidate`, `open` | MCP policy/session interface, runtime architecture, concurrency/security conformance. Select the v1 reuse policy in `DR-044`. |
| `REQ-030` | Every selected runtime root is canonicalized with platform-aware link/alias handling and must be contained in an explicitly authorized project/workspace root before backend access. A deny-list is not the primary boundary, and MCP roots do not substitute for OS sandboxing. | Supplement deny-list proposal at `supplement:L17`, corrected by security review | `candidate` | Runtime security interface, Windows/Linux fixtures, conformance, and failure injection. Approve or revise `DR-045` separately from root selection. |
...
| `GAP-023` | Independent server-tool policy guarantees and defaults: discovery pagination/change handling, call-denial error/audit schema, backend non-delivery proof, unmanaged-route boundary, and whether sampling-driven tool use, prompts, or resources are outside v1 policy or separately mediated. | `REQ-028`, MCP policy conformance |
| `GAP-024` | MCP client-session isolation and backend-sharing eligibility: initialization/capabilities, request namespaces, callbacks, roots, transport-specific auth, unique single-use OAuth state/per-flow PKCE, resource/audience binding, consent/token storage/no-passthrough policy, cancellation/progress/tasks, notifications, logging, reconnect, and cleanup. | `REQ-012`, `REQ-029`, runtime conformance |
| `GAP-025` | Runtime project-root input ordering, pre-initialization identity, post-initialization root acquisition/change lifecycle, multi-root selection, and failure when the intended root is ambiguous or unavailable. | `REQ-011`, runtime conformance |
| `GAP-026` | Authorized-root representation, canonicalization across Windows/Linux links and aliases, containment enforcement, active-session response to root narrowing/removal, and distinction between MCP roots and OS sandboxing. | `REQ-030`, runtime security conformance |
