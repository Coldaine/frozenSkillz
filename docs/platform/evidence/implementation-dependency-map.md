...
| `PHASE-1-DECISIONS` | Close authority/lifecycle, install surfaces, project-vs-global precedence, manifest/lock names, CLI surface, rule-template policy, supported-client matrix, versioning, tool-policy threat model, MCP backend-sharing policy, root selection/security, and non-goal decisions. | `PHASE-0-EVIDENCE` complete. | `DEC-GAP-001` through `DEC-GAP-006`, `GAP-001` through `GAP-005`, `GAP-008` through `GAP-011`, and `GAP-023` through `GAP-026` have explicit approved outcomes or explicit deferrals with no dependent v1 work. | No schema, CLI, managed-proxy boundary, root behavior, or reuse behavior is declared stable while its governing choice remains open. |
...
| `PHASE-4-CLIENT-RUNTIME` | Implement machine-global and project-local discovery adapters, supported native MCP renderers, trust/approval observation, namespace/collision analysis, gateway wrapper, and only retained tool-policy/session-sharing/root-security capabilities, plus Windows/Linux launcher fixtures. | `PHASE-3-LOCAL-CORE` APIs are stable enough for adapters and every retained MCP policy/session/root contract is approved. | All declared client/version fixtures pass behavior tests; portable/host/project-process MCP categories pass required root, pin, availability, health, policy, session-isolation, and concurrency tests on deterministic environments. | No claim of client parity, hard security, or safe backend sharing without passing exact-version/backend fixtures; deferred capabilities are not implemented implicitly. |
...
| `GATE-MCP-POLICY-SESSION` | `PHASE-4-CLIENT-RUNTIME` | Managed versus unmanaged traffic boundary; separately selected discovery filtering and call authorization; errors/audit/redaction; per-client protocol sessions; backend-sharing eligibility; transport-specific auth/root/callback/cancellation/notification isolation; daemon peer security; root-source lifecycle and authorized-root containment if retained. Traces to `REQ-011`, `REQ-028` through `REQ-030`, `GAP-023` through `GAP-026`. |
...
| `BL-006` Implement the gateway wrapper and only the approved tool-discovery, call-authorization, session-mediation, backend-reuse, root-lifecycle/containment, and runtime-observation capabilities. | `pasted-text.txt:L739-841`; supplemental review | `BL-004`, approved runtime/observation contracts, `GATE-MCP-POLICY-SESSION` | `PHASE-4-CLIENT-RUNTIME` |
