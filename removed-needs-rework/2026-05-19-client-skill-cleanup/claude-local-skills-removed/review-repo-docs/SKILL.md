...
description: Review and fix all agent documentation (CLAUDE.md, AGENTS.md, README, docs/) by validating against actual codebase state. Finds stale references, contradictions, and gaps.
user_invocable: true
triggers:
  - review docs
  - fix documentation
...
```bash
find . -maxdepth 3 \( -name "CLAUDE.md" -o -name "AGENTS.md" -o -name "README*" -o -name "docs" -type d \) -not -path "*/node_modules/*" -not -path "*/.git/*" 2>/dev/null
cat CLAUDE.md AGENTS.md README.md 2>/dev/null | head -200
```

Also check global docs:
```bash
cat ~/.claude/CLAUDE.md ~/.claude/AGENTS.md 2>/dev/null | head -100
```
...
- **CLAUDE.md mentions file X** but **AGENTS.md says file Y**
...
- [ ] CLAUDE.md line X: mentions "docs/thing.md" — file does not exist
...
- [ ] CLAUDE.md says tech stack includes Redis, AGENTS.md doesn't mention it
...
- [ ] `configs/` directory mentioned in AGENTS.md but no CLAUDE.md section for it
...
FILES CHECKED:
- CLAUDE.md: X references validated
...
✓ Tech stack match (package.json ↔ CLAUDE.md declarations)

## What This Does NOT Do

- Generate generic templates (use `claude-md-enhancer` for that)
...
Works with:
- `claude-md-enhancer` (after reviewing, enhance structure if needed)
