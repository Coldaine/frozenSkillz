...
| Post reply | `gh api repos/{owner}/{repo}/pulls/{pr}/comments --method POST --field body="..." --field in_reply_to={id}` |
...
- `in_reply_to` - Parent comment ID (for replies via `gh api`)
...
gh api repos/{owner}/{repo}/pulls/{pr}/comments \
  --method POST \
  --field body="✅ Fixed" \
  --field in_reply_to=123
```

### Resolve Review Thread (Preferred)
...
**Note:** Use GraphQL thread IDs, not REST comment IDs.
...
gh api repos/{owner}/{repo}/pulls/{pr}/comments \
  --jq '[.[] | select(.in_reply_to_id == null)] | map({
    id,
    path,
    replies: (.id as $root | [.[] | select(.in_reply_to_id == $root)] | length)
  })'
```
