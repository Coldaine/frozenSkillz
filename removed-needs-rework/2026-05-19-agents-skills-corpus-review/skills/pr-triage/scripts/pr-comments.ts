...
function isBotAuthor(author: string): boolean {
	return BOT_AUTHORS_SET.has(author);
}
...
export async function getThreadId(
	...
	const query = `
		query GetThreadId($owner: String!, $name: String!, $pr: Int!, $cursor: String) {
	...
	for (;;) {
		...
		for (const thread of nodes) {
			const matches = thread?.comments?.nodes?.some(
				(comment: { databaseId?: number }) => comment?.databaseId === commentId,
			);
			if (matches) return thread.id;
		}
...
export async function summarizeByFile(
	...
	return Array.from(byFile.entries()).map(([file, fileComments]) => ({
		...
		hasHuman: fileComments.some((c) => !isBotAuthor(c.author)),
	}));
}
...
export function triage(comments: CommentMetadata[]): TriagedComment[] {
	return comments
		.map((c) => {
			const isBot = isBotAuthor(c.author);
			const isRoot = c.inReplyToId === null;

			// Treat all comments as high-priority regardless of author.
			const needsBody = true;
		...
		.sort((a, b) => {
			// Priority: root > reply (no author bias)
			if (a.isRoot && !b.isRoot) return -1;
			if (!a.isRoot && b.isRoot) return 1;
			// Newer comments first within same class.
...
export async function mergeGate(
	repo: string,
	pr: number,
): Promise<{ ok: boolean; unreplied: CommentMetadata[] }> {
	const comments = await fetchMetadata(repo, pr);
	...
	return { ok: unreplied.length === 0, unreplied };
}
...
async function main() {
	...
	switch (command) {
		...
		case "unreplied": {
			...
			const result = await mergeGate(repo, Number(pr));
			console.log(JSON.stringify(result.unreplied, null, 2));
			break;
		}
		...
		case "gate": {
			const [repo, pr] = args;
			if (!repo || !pr) {
				console.error("Usage: pr-comments.ts gate <owner/repo> <pr>");
				process.exit(1);
			}
			const result = await mergeGate(repo, Number(pr));
			console.log(JSON.stringify(result, null, 2));
...
Commands:
  ...
  triage <owner/repo> <pr>            Triage with root-first sorting (no author bias)
  unreplied <owner/repo> <pr>         List root comments without replies
  gate <owner/repo> <pr>              Exit non-zero if unresolved root comments remain
